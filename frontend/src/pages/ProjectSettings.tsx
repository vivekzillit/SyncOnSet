...
export default function ProjectSettings() {
  ...
  const [f, setF] = useState({ name: "", code: "", status: "PREP", shootingDay: "0", currentLocation: "", currency: "INR", prepStartDate: "", prepEndDate: "", prepWrapDate: "", startDate: "", endDate: "", wrapDate: "", notes: "" });
  useEffect(() => { if (project) setF({ name: project.name, code: project.code, status: project.status, shootingDay: String(project.shootingDay ?? 0), currentLocation: project.currentLocation || "", currency: project.currency, prepStartDate: project.prepStartDate ? toLocalInput(project.prepStartDate).slice(0, 10) : "", prepEndDate: project.prepEndDate ? toLocalInput(project.prepEndDate).slice(0, 10) : "", prepWrapDate: project.prepWrapDate ? toLocalInput(project.prepWrapDate).slice(0, 10) : "", startDate: project.startDate ? toLocalInput(project.startDate).slice(0, 10) : "", endDate: project.endDate ? toLocalInput(project.endDate).slice(0, 10) : "", wrapDate: project.wrapDate ? toLocalInput(project.wrapDate).slice(0, 10) : "", notes: project.notes || "" }); }, [project]);
  const save = useMutation({
    mutationFn: () => api(`/projects/${projectId}`, { method: "PATCH", body: { ...f, shootingDay: Number(f.shootingDay), prepStartDate: f.prepStartDate || null, prepEndDate: f.prepEndDate || null, prepWrapDate: f.prepWrapDate || null, startDate: f.startDate || null, endDate: f.endDate || null, wrapDate: f.wrapDate || null } }),
    ...
    <div>
      ...
      <Card>
        <div className="form-grid">
          ...
          <div />
          <Field label="Prep start"><Input type="date" value={f.prepStartDate} onChange={(e) => setF({ ...f, prepStartDate: e.target.value })} /></Field>
          <Field label="Prep end"><Input type="date" value={f.prepEndDate} min={f.prepStartDate || undefined} onChange={(e) => setF({ ...f, prepEndDate: e.target.value })} /></Field>
          <Field label="Prep wrap"><Input type="date" value={f.prepWrapDate} min={f.prepStartDate || undefined} onChange={(e) => setF({ ...f, prepWrapDate: e.target.value })} /></Field>
          ...
          <Field label="Wrap date" help="Last day of principal photography"><Input type="date" value={f.wrapDate} min={f.startDate || undefined} onChange={(e) => setF({ ...f, wrapDate: e.target.value })} /></Field>

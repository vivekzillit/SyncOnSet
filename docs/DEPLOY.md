...
| `ANTHROPIC_API_KEY` | unset | Optional. Costume cues work without it via the built-in reader; with it, AI (`claude-opus-5`; `ANTHROPIC_MODEL` overrides) reads scenes instead. Set as a secret environment variable, never in the repo |

...
2. **Scenes → Upload schedule** and **Upload callsheet** work exactly like the script upload, but for shoot dates. Drop in the shooting schedule (a one-line schedule PDF from Movie Magic, StudioBinder or Celtx, or a CSV with Scene and Date columns) or the day's callsheet, and the shoot dates are read straight from it. You get the same kind of preview: one row per scene with its date, which you can edit, untick anything you do not want, then **Apply**. Those scenes get their shoot date and turn **Scheduled**, so they show under **Today** and **Upcoming**. Only the shoot date changes, and nothing is saved until you press Apply. Tomorrow's advance block on a callsheet is dated the next day, not today, and scenes named only in continuity notes ("same saree as Sc 4") are left alone. Dates written as 03/04/2026 are read as day/month and flagged; scene numbers that are not in the breakdown are listed and skipped.

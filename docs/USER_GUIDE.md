...
2. **Scenes → Upload schedule** and **Upload callsheet** give scenes their shoot dates. Drop in the shooting schedule (the one-line schedule PDF from Movie Magic, StudioBinder or Celtx, or a CSV with Scene and Date columns) or the day's callsheet (PDF or text). The app reads each shoot day's date and scene numbers and shows them for review: fix any date, see which scene numbers are not in the breakdown, then **Apply**. Scenes get their shoot date and turn **Scheduled**; the **Today** and **Upcoming** filters on the Scenes page then show that day's work, and the uploaded document stays linked above the table. Dates written as 03/04/2026 are read as day/month.

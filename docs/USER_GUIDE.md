...
1. **Projects → Create a Production**. A five-step wizard asks what you are working on (**Feature** or **TV Series**), the title, and the estimated dates: prep start, end and wrap, then shoot start, end and wrap — every date optional. A TV Series carries episodes: each scene has an **Ep** field, and Scenes and Breakdown gain an episode column and filter. **Upload script for breakdown** follows: name the draft (for example "White" or "Blue") and drop in the screenplay, or press **Continue** to create the production without one and upload the script later from the Scenes page.
